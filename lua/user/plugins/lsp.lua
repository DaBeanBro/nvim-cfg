return {
  ...
  config = function()
      ...
      yamlls = {
        settings = {
          yaml = {
            ...
            customTags = { '!Equals sequence', '!FindInMap sequence', '!GetAtt', '!GetAZs', '!ImportValue', '!Join sequence', '!Ref', '!Select sequence', '!Split sequence', '!Sub', '!Or sequence' },
          }
        }
      },
      eslint = {
    ...
    local special_server_configs = {
      tsserver = function() typescript.setup({ server = tsserver_config }) end,
      zk = disable,
    ...
    map('n', 'gd', function() telescope.lsp_definitions() end, 'Goto definition')
    map('n', 'gD', lsp.buf.declaration, 'Goto declaration')
    map('n', 'gi', function() telescope.lsp_implementations() end, 'Goto implementation')
    map('n', 'go', function() telescope.lsp_type_definitions() end, 'Goto type definition')
    map('n', 'gr', function() telescope.lsp_references() end, 'Goto references')
    ...
    map('n', '<leader>ld', function() diagnostic.open_float() end, 'Show diagnostics')
    map('n', '[d', function() diagnostic.goto_prev() end, 'Previous diagnostic')
    map('n', ']d', function() diagnostic.goto_next() end, 'Next diagnostic')
